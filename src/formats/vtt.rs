use crate::{config::Config, formats::time::format_vtt_timestamp, model::Transcript};
use textwrap::wrap;

pub fn write_vtt(t: &Transcript, cfg: &Config) -> String {
    ...
    for cue in &t.cues {
        ...
        for line in wrap(&text, cfg.formats.vtt.wrap_width) {
            out.push_str(&line);
            out.push('\n');
        }
